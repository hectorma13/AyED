...
{
  T resultado_producto{0};
  assert(v.get_size() == w.get_size());
  for (int i{0}; i <  v.get_size(); i++) {
    resultado_producto = resultado_producto + (v[i] * w[i]);
  }
...
{
  double resultado_producto{0.0};
  assert(v.get_size() == w.get_size());
  for (int i{0}; i <  v.get_size(); i++) {
    resultado_producto = resultado_producto + (v[i].value() * w[i].value());
  }
  return resultado_producto;
}